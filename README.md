...
- 菊韻粵語：[rime-gukwan: 菊韻粵語輸入方案 基於中州韻 Gukwan Cantonese IME (RIME Scheme)](https://github.com/HoengSaan/rime-gukwan)
...
此爲[RIME | 中州韻輸入法引擎](https://rime.im/)諺文方案，基於[oniondelta/Hangul_Rime_Files](https://github.com/oniondelta/Hangul_Rime_Files)。造輪目的是因爲我不甚鍾意其他諺文羅馬字方案，創造基準爲一個部件（非諺文字母）對應一個拉丁字母。
...
同文用戶可使用[菊韻同文主題](https://github.com/HoengSaan/trime-gukwan/)，有適配佈局。

Trime User can use [trime-gukwan](https://github.com/HoengSaan/trime-gukwan/) theme for the dedicated layout.
...
- 後綴`w`出諺文字母（終聲專用）｜Subfix `w` for Hangul Jamo ( Only Final )
- 前綴`=`倉頡反查｜Prefix `=` for Cangjie reverse look-up
- `x`爲去聲；`xx`爲上聲｜`x` for departing tone; `xx` for rising tone.

### 輸入 Input

- 此方案中，「ㅕ」「ㅑ」「ㅠ」「ㅛ」被視爲兩個部件。絕大多數複合字母均被視爲多個部件。；In this scheme, 「ㅕ」「ㅑ」「ㅠ」「ㅛ」 are treated as two components. Most composed jamos are treated as multiple components.
  - ㅑ：ㅣ＋ㅏ＝i+a＝ia
  - ㅒ：ㅣ+ㅏ+ㅣ＝i+a+i＝iai
...
- `oa`可用`ua`替代｜`oa` can be replaced with `ua`
